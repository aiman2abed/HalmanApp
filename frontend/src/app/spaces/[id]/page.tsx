...
const spaceData: Record<string, SpaceInfo> = {
  realistic: {
    ...
    description:
      "المكان المفضل للممارسين وصناع الأشياء! هنا نحول الأفكار إلى آلات تتحرك.",
    careers: ["مهندس ميكانيكي", "مطور روبوتات", "فني إلكترونيات"],
    projects: [
      "بناء ذراع آلية هيدروليكية",
      "تصليح الألعاب الإلكترونية المعطلة",
    ],
  },
  investigative: {
    id: "investigative",
    ...
    description:
      "ملاذ المفكرين والباحثين. هنا نطرح الأسئلة الصعبة ونبحث عن الإجابات عبر التجارب.",
  ...
  artistic: {
    ...
    description:
      "مساحة المبدعين والفنانين. لا حدود للخيال هنا، من الرسم إلى تأليف الموسيقى والبودكاست.",
  ...
  social: {
    ...
    description:
      "قلب المدرسة النابض. مكان يجمع الأشخاص الذين يحبون مساعدة الآخرين وصنع تأثير إيجابي.",
  ...
  enterprising: {
    ...
    description:
      "موطن القادة والمبادرين. هنا نتعلم كيف ندير المشاريع ونقنع الآخرين بأفكارنا.",
    careers: ["رائد أعمال", "مدير مشاريع", "محامي"],
    projects: [
      "تأسيس شركة صغيرة لبيع الأعمال اليدوية",
      "إدارة حملة لانتخابات المدرسة",
    ],
  },
  conventional: {
    id: "conventional",
  ...
  return (
    <div className="flex flex-col w-full h-full pb-20">
      ...
      <div className="flex items-center justify-between p-4 sticky top-0 z-20 bg-slate-50/80 backdrop-blur-md">
        <Link
          href="/"
          className="bg-white p-2 rounded-full shadow-sm border border-slate-200 hover:bg-slate-100 transition-colors"
        >
      ...
      <div className="px-4 md:px-6 space-y-6 max-w-2xl mx-auto w-full">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h1 className="text-3xl md:text-4xl font-black mb-2 drop-shadow-md">
              {space.name}
            </h1>
        ...
        >
          ...
          <div className="bg-orange-50 border-2 border-dashed border-orange-200 rounded-2xl p-6 text-center">
            ...
            <p className="text-sm font-bold text-orange-600">
              منطقة الألعاب المصغرة (قيد التطوير)
            </p>
            <p className="text-xs text-orange-500 mt-1">
              ستتوفر تحديات {space.name} التفاعلية هنا قريباً!
            </p>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="space-y-3">
            {space.projects.map((project: string, index: number) => (
              <div
                key={index}
                className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-sky-200 transition-colors group cursor-pointer"
              >
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            {space.careers.map((career: string, index: number) => (
              <span
                key={index}
                className="bg-emerald-50 text-emerald-700 border border-emerald-100 px-4 py-2 rounded-xl text-sm font-bold"
              >
                {career}
              </span>
            ))}
